...
export function AddPropertyExpensePage() {
    const navigate = useNavigate();
    const [searchParams] = useSearchParams();
    const defaultPropertyId = searchParams.get('propertyId');

    const { formData, setFormData, handleSubmit } = usePropertyExpenseForm();
    ...
    const onSubmit = async (e: React.FormEvent) => {
        ...
        navigate(`/property-expenses${defaultPropertyId && defaultPropertyId !== 'all' ? `?propertyId=${defaultPropertyId}` : ''}`);
    };

    return (
        <AppLayout header={<Header title="Add Property Expense" leftElement={<button onClick={() => navigate(`/property-expenses${defaultPropertyId && defaultPropertyId !== 'all' ? `?propertyId=${defaultPropertyId}` : ''}`)}><Icon name="arrow_back" className="text-2xl" /></button>} />}>
                ...
                <form onSubmit={onSubmit} className="bg-white dark:bg-surface-dark rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-[#283933] space-y-6">
                    <div className="flex items-center gap-4 border-b border-slate-100 dark:border-[#283933] pb-6">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-slate-500 dark:text-slate-400">Record a new expense for your property.</p>
                        </div>
                    </div>
                    ...
                    <div className="pt-6 border-t border-slate-100 dark:border-[#283933] flex justify-end gap-3">
                        <button
                            type="button"
                            onClick={() => navigate(`/property-expenses${defaultPropertyId && defaultPropertyId !== 'all' ? `?propertyId=${defaultPropertyId}` : ''}`)}
