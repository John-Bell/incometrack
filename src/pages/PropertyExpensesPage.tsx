...
export function PropertyExpensesPage() {
    ...
    return (
        <AppLayout header={<Header title="Property Expenses" />}>
            <div className="max-w-4xl mx-auto space-y-6 p-4">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                    <h2 className="text-2xl font-semibold text-slate-800 dark:text-white flex items-center gap-3">
                         <div className="w-10 h-10 bg-rose-50 dark:bg-rose-900/20 rounded-xl flex items-center justify-center text-rose-500 dark:text-rose-400">
                    ...
                    <div className="flex flex-col sm:flex-row w-full sm:w-auto gap-3">
                        ...
                        <Link
                            to={`/property-expenses/add${propertyIdFilter !== 'all' ? `?propertyId=${propertyIdFilter}` : ''}`}
                ...
                {!expenses || expenses.length === 0 ? (
                    <div className="text-center py-12 bg-white dark:bg-surface-dark rounded-2xl shadow-sm border border-slate-100 dark:border-[#283933]">
                        ...
                        <h3 className="text-lg font-medium text-slate-900 dark:text-white mb-2">No Expenses Found</h3>
                        <p className="text-slate-500 dark:text-slate-400 mb-6 max-w-sm mx-auto">
                            {propertyIdFilter === 'all'
                                ? "You haven't added any property expenses yet."
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-slate-50/50 dark:bg-black/20 text-xs uppercase tracking-wider text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-[#283933]">
                                        ...
                                        <th className="px-6 py-4 font-semibold w-1/4">Payee</th>
                                ...
                                <tbody className="divide-y divide-slate-100 dark:divide-[#283933]">
                                    {expenses.map(expense => (
                                        <tr key={expense.id} className="hover:bg-slate-50/50 dark:hover:bg-black/10 transition-colors">
                                            ...
                                            <td className="px-6 py-4">
                                                <p className="text-sm font-medium text-slate-900 dark:text-white truncate">{expense.payee}</p>
                                                {expense.description && (
                                                    <p className="text-xs text-slate-500 dark:text-slate-400 truncate mt-0.5">{expense.description}</p>
                                                )}
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap text-right">
                                                <span className="inline-flex items-center px-2.5 py-1 rounded-full text-sm font-medium bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400">
                                                    {formatCurrency(expense.amount)}
                                                </span>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap text-right">
                                                <Link
                                                    to={`/property-expenses/edit/${expense.id}?propertyId=${propertyIdFilter}`}
